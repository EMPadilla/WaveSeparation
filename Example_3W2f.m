...
[Amp.Total,~] = FUNCTION_AmplitudePhase(eta,time,f1x2); close
[Amp.Th_IBW,~] = FUNCTION_AmplitudePhase(Th_IBW,time,f1x2); close
[Amp.Th_IFW,~] = FUNCTION_AmplitudePhase(Th_IFW,time,f1x2); close
[Amp.Th_OFW,~] = FUNCTION_AmplitudePhase(Th_OFW,time,f1x2); close

%% Propagation characteristics:

% Propagation characteristics for the bound wave
Kinematic.BoundAprox.k = FUNCTION_DispersionEq(d,1/f1);
Kinematic.BoundAprox.c = (2*pi*f1)./Kinematic.BoundAprox.k; 
Kinematic.BoundAprox.Cg = Kinematic.BoundAprox.c./2.*(1+(2.*Kinematic.BoundAprox.k.*d)./sinh(2.*Kinematic.BoundAprox.k.*d));
Kinematic.BoundAprox.tau = cumsum(abs(horzcat(0,diff(x)))./Kinematic.BoundAprox.c);
Kinematic.BoundAprox.Keq = (2*pi*f1x2).*Kinematic.BoundAprox.tau./abs(x-x(1));
Kinematic.BoundAprox.Keq(1) = (2*pi*f1x2)/(2*pi*f1)*Kinematic.BoundAprox.k(1);

% Propagation characteristics for the free waves at 2f1
[Kinematic.Free.f1x2] = FUNCTION_Kinematic_FreeWaves(x,d,f1x2);

%% Quantitative Analysis: Wave-Separation    
Vbound = Kinematic.BoundAprox.c;
[IBW.P4_Psep2,OBW.P4_Psep2,OFW.P4_Psep2,IFW.P4_Psep2] = FUNCTION_WaveSeparation4W(etaNoise,x,d,Vbound,f1x2,fs,[4 2]);
[IBW.P4_Psep6,OBW.P4_Psep6,OFW.P4_Psep6,IFW.P4_Psep6] = FUNCTION_WaveSeparation4W(etaNoise,x,d,Vbound,f1x2,fs,[4 6]);


%% Theoretical estimation of the nodes and antinodes 

WaveNumbers(1,:) = Kinematic.BoundAprox.Keq * cos(0);
WaveNumbers(2,:) = Kinematic.Free.f1x2.Keq * cos(pi); 
[xAnti.IBW_OFW,xNodes.IBW_OFW,L_Approx.IBW_OFW] = FUNCTION_xAntinodes2D(x,[IBW.P4_Psep6.Ph0 OFW.P4_Psep6.Ph0],WaveNumbers);

WaveNumbers(1,:) = Kinematic.BoundAprox.Keq * cos(0);
WaveNumbers(2,:) = Kinematic.Free.f1x2.Keq * cos(0); 
[xAnti.IBW_IFW,xNodes.IBW_IFW,L_Approx.IBW_IFW] = FUNCTION_xAntinodes2D(x,[IBW.P4_Psep6.Ph0 IFW.P4_Psep6.Ph0],WaveNumbers);
...
ax1 = subplot(2,2,1); 
    clear p
    p(7) = area([1.5; 1.5+L_Approx.IBW_IFW],[0.02; 0.02],0,'LineStyle','none');hold on; p(7).FaceColor = [0.85 0.85 0.85];hold on
    p(8) = area([4.38; 4.38+L_Approx.IBW_OFW],[0.02; 0.02],0,'LineStyle','none');hold on; p(8).FaceColor = [0.7 0.7 0.7];hold on
    p(1) = plot(x,Amp.Total,'.:','Color',[0 0.4470 0.7410],'markersize',8);hold on
    p(2) =plot(x,Amp.Th_IBW,'k-');
    p(3) =plot(x,Amp.Th_OFW,'r-');
    p(4) =plot(x,Amp.Th_IFW,'-g');
    p(5) = plot(xAnti.IBW_OFW,interp1(x,Amp.Total,xAnti.IBW_OFW),'r.','markersize',22);
    p(6) = plot(xNodes.IBW_IFW,interp1(x,Amp.Total,xNodes.IBW_IFW),'k.','markersize',25);
    legend(p,'Total','IBW','OFW','IFW','$X_{anti}[IBW,OFW]$','$X_{node}[IBW,IFW]$','$L_{IBW,IFW}$','$L_{IBW,OFW}$','Location','southeastoutside')
    ylim([0 0.004])
    xlim([x(1) x(end)])
    ylabel('Wave Amplitude (m)')
    set(gca,'fontsize',12)
    grid on 
    text(0.5,3.6e-3,'a.','FontSize',18)
...
ax2 = subplot(2,2,2);
    ...
    p(2) = plot(x,Kinematic.Free.f1x2.tau-0.3,'-g','LineWidth',2);hold on
    p(1) = plot(x,Kinematic.BoundAprox.tau-0.3,'k-','LineWidth',2);hold on
    p(3) = plot(x,-Kinematic.Free.f1x2.tau+12.4,'r-','LineWidth',2);hold on
    legend(p,'IBW','IFW','OFW','Location','West')
    ylim([0 12])
    xlim([x(1) x(end)])
    ylabel('Time (s)')
    set(gca,'fontsize',12)
...
ax1 = subplot(2,2,1);
    clear p
    p(1) = plot(x,Amp.Total,'.:','Color',[0 0.4470 0.7410],'markersize',8);hold on
    p(2) = plot(x,IBW.P4_Psep2.Amp,'.k','markersize',8);
    p(3) = plot(x,Amp.Th_IBW,'k--');
    p(4) = plot(x,OBW.P4_Psep2.Amp,'.','Color',[0.93 0.69 0.13],'markersize',8);
    p(5) = plot(x,OFW.P4_Psep2.Amp,'.r','markersize',8);
    p(6) = plot(x,Amp.Th_OFW,'r--');
    p(7) = plot(x,IFW.P4_Psep2.Amp,'.g','markersize',8);
    p(8) = plot(x,Amp.Th_IFW,'--g');
    legend(p,'Total','IBW (Sep)','IBW (Th)','OBW (Sep)','OFW (Sep)','OFW (Th)','IFW (Sep)','IFW (Th)','Location','north')
    xlim([x(1) x(end)])
    ylabel('Wave Amplitude (m)')
    grid on
    set(gca,'fontsize',12)
    text(0.75,3.6e-3,'a.','FontSize',18)
    
ax2 = subplot(2,2,2);
    plot(IBW.P4_Psep2.alpha,'.:','Color',[0 0.4470 0.7410],'markersize',12);hold on
    ylabel('$\alpha$')
    ylim([0.85 1.05])
    xlim([1 6])
    grid on
    set(gca,'fontsize',12)
    text(1.3,1.03,'b.','FontSize',18)
    
ax3 = subplot(2,2,3);
    clear p
    p(1) = plot(x,Amp.Total,'.:','Color',[0 0.4470 0.7410],'markersize',8);hold on
    p(2) = plot(x,IBW.P4_Psep6.Amp,'.k','markersize',8);
    p(3) = plot(x,Amp.Th_IBW,'k--');
    p(4) = plot(x,OBW.P4_Psep6.Amp,'.','Color',[0.93 0.69 0.13],'markersize',8);
    p(5) = plot(x,OFW.P4_Psep6.Amp,'.r','markersize',8);
    p(6) = plot(x,Amp.Th_OFW,'r--');
    p(7) = plot(x,IFW.P4_Psep6.Amp,'.g','markersize',8);
    p(8) = plot(x,Amp.Th_IFW,'--g');
    legend(p,'Total','IBW (Sep)','IBW (Th)','OBW (Sep)','OFW (Sep)','OFW (Th)','IFW (Sep)','IFW (Th)','Location','north')
    xlim([x(1) x(end)])
    ylabel('Wave Amplitude (m)')
    ...
    grid on
    set(gca,'fontsize',12)
    text(0.75,3.6e-3,'c.','FontSize',18)

ax4 = subplot(2,2,4);
    clear p
    p(1) = plot(IBW.P4_Psep6.alpha,'.:','Color',[0 0.4470 0.7410],'markersize',12);hold on
    ylabel('$\alpha$')
    xlabel('Iteration')
    ylim([0.85 1.05])
    xlim([1 6])
