...
[IBW.P1_Psep1,OFW.P1_Psep1,IFW.P1_Psep1] = FUNCTION_WaveSeparation(etaNoise,x,d,Vbound,f1x2,fs,[1 1]);
[IBW.P1_Psep6,OFW.P1_Psep6,IFW.P1_Psep6] = FUNCTION_WaveSeparation(etaNoise,x,d,Vbound,f1x2,fs,[1 6]);

%% Theoretical estimation of the nodes and antinodes 
celerities(1,:) = -Kinematic.BoundAprox.c;
celerities(2,:) = Kinematic.Free.f1x2.c; 
[xAnti.IBW_OFW,xNodes.IBW_OFW,L_Approx.IBW_OFW] = FUNCTION_xAntinodes(x,f1x2,[IBW.P1_Psep6.Ph0 OFW.P1_Psep6.Ph0],celerities);

celerities(1,:) = -Kinematic.BoundAprox.c;
celerities(2,:) = -Kinematic.Free.f1x2.c; 
[xAnti.IBW_IFW,xNodes.IBW_IFW,L_Approx.IBW_IFW] = FUNCTION_xAntinodes(x,f1x2,[IBW.P1_Psep6.Ph0 IFW.P1_Psep6.Ph0],celerities);

celerities(1,:) = -Kinematic.Free.f1x2.c;
celerities(2,:) = Kinematic.Free.f1x2.c; 
[xAnti.IFW_OFW,xNodes.IFW_OFW,L_Approx.IFW_OFW] = FUNCTION_xAntinodes(x,f1x2,[IFW.P1_Psep6.Ph0 OFW.P1_Psep6.Ph0],celerities);
...
ax1 = subplot(2,2,1) 
    ...
    p(4) =plot(x,Amp.Th_IFW,'-','Color',[0.93 0.69 0.13])
...
ax2 = subplot(2,2,2)
    ...
    p(2) = plot(x,Kinematic.Free.f1x2.tau-0.3,'-','Color',[0.93 0.69 0.13],'LineWidth',2);hold on
...
ax1 = subplot(2,2,1)
    clear p
    p(1) = plot(x,Amp.Total,'.:','Color',[0 0.4470 0.7410],'markersize',8);hold on
    p(2) =plot(x,IBW.P1_Psep1.Amp,'.k','markersize',11)
    p(3) =plot(x,Amp.Th_IBW,'k--')
    p(4) =plot(x,OFW.P1_Psep1.Amp,'.r','markersize',11)
    p(5) =plot(x,Amp.Th_OFW,'r--')
    p(6) =plot(x,IFW.P1_Psep1.Amp,'.','Color',[0.93 0.69 0.13],'markersize',11)
    p(7) =plot(x,Amp.Th_IFW,'--','Color',[0.93 0.69 0.13])
    legend(p,'Total','ILW (Sep)','ILW (Th)','OFLW (Sep)','OFLW (Th)','IFLW (Sep)','IFLW (Th)','Location','north')
    xlim([x(1) x(end)])
    ylabel('Wave Amplitude (m)')
    grid on
    set(gca,'fontsize',12)
...
ax2 = subplot(2,2,2)
    clear p
    p(1) = plot(IBW.P1_Psep1.alpha,'.:','Color',[0 0.4470 0.7410],'markersize',12);hold on
    ylabel('$\alpha$')
    ylim([0.85 1.05])
    xlim([1 6])
    grid on
...
ax3 = subplot(2,2,3)
    clear p
    p(1) = plot(x,Amp.Total,'.:','Color',[0 0.4470 0.7410],'markersize',8);hold on
    p(2) =plot(x,IBW.P1_Psep6.Amp,'.k','markersize',11)
    p(3) =plot(x,Amp.Th_IBW,'k--')
    p(4) =plot(x,OFW.P1_Psep6.Amp,'.r','markersize',11)
    p(5) =plot(x,Amp.Th_OFW,'r--')
    p(6) =plot(x,IFW.P1_Psep6.Amp,'.','Color',[0.93 0.69 0.13],'markersize',11)
    p(7) =plot(x,Amp.Th_IFW,'--','Color',[0.93 0.69 0.13])
    legend(p,'Total','ILW (Sep)','ILW (Th)','OFLW (Sep)','OFLW (Th)','IFLW (Sep)','IFLW (Th)','Location','north')
    xlim([x(1) x(end)])
    ylabel('Wave Amplitude (m)')
...
ax4 = subplot(2,2,4)
    clear p
    p(1) = plot(IBW.P1_Psep6.alpha,'.:','Color',[0 0.4470 0.7410],'markersize',12);hold on
    ylabel('$\alpha$')
    ylim([0.85 1.05])
    xlim([1 6])
    grid on
