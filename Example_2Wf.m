...
[IBW.P1_Psep1,OFW.P1_Psep1,IFW.P1_Psep1] = FUNCTION_WaveSeparation(etaNoise,x,d,Vbound,f1,fs,[1 1]);
[IBW.P1_Psep6,OFW.P1_Psep6,IFW.P1_Psep6] = FUNCTION_WaveSeparation(etaNoise,x,d,Vbound,f1,fs,[1 6]);

%% Theoretical estimation of the nodes and antinodes 
celerities(1,:) = -Kinematic.Free.f1.c;
celerities(2,:) = Kinematic.Free.f1.c; 
[xAnti.IFW_OFW,xNodes.IFW_OFW,L_Approx.IFW_OFW] = FUNCTION_xAntinodes(x,f1,[IFW.P1_Psep6.Ph0 OFW.P1_Psep6.Ph0],celerities);
...
ax1 = subplot(2,2,1) 
    ...
    p(3) =plot(x,Amp.Th_IFW,'-','Color',[0.93 0.69 0.13])
...
ax2 = subplot(2,2,2)
    ...
    p(1) = plot(x,Kinematic.Free.f1.tau-1,'-','Color',[0.93 0.69 0.13],'LineWidth',2);hold on
...
ax1 = subplot(2,2,1)
    ...
    p(3) =plot(x,OFW.P1_Psep1.Amp,'.r','markersize',11)
    p(4) =plot(x,Amp.Th_OFW,'r--')
    p(5) =plot(x,IFW.P1_Psep1.Amp,'.','Color',[0.93 0.69 0.13],'markersize',11)
    p(6) =plot(x,Amp.Th_IFW,'--','Color',[0.93 0.69 0.13])
    legend(p,'Total','IBW (Sep)','OFW (Sep)','OFW (Th)','IFW (Sep)','IFW (Th)','Location','north')
    xlim([x(1) x(end)])
    ylim([0 0.012])
    ylabel('Wave Amplitude (m)')
    grid on
...
ax3 = subplot(2,2,3)
    ...
    p(3) =plot(x,OFW.P1_Psep6.Amp,'.r','markersize',11)
    p(4) =plot(x,Amp.Th_OFW,'r--')
    p(5) =plot(x,IFW.P1_Psep6.Amp,'.','Color',[0.93 0.69 0.13],'markersize',11)
    p(6) =plot(x,Amp.Th_IFW,'--','Color',[0.93 0.69 0.13])
    legend(p,'Total','IBW (Sep)','OFW (Sep)','OFW (Th)','IFW (Sep)','IFW (Th)','Location','north')
    xlim([x(1) x(end)])
    ylim([0 0.012])
