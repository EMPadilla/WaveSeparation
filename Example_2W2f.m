...
[IBW.P1_Psep1,OFW.P1_Psep1,IFW.P1_Psep1] = FUNCTION_WaveSeparation(etaNoise,x,d,Vbound,f1x2,fs,[1 1]);   %Local Array with [P=1; Psep = 1]
[IBW.P1_Psep6,OFW.P1_Psep6,IFW.P1_Psep6] = FUNCTION_WaveSeparation(etaNoise,x,d,Vbound,f1x2,fs,[1 6]);   %Local Array with [P=1; Psep = 6]
...
ax1 = subplot(2,2,1) 
    clear p
    p(5) = area([xAnti.IBW_OFW(1); xAnti.IBW_OFW(1)+L_Approx.IBW_OFW],[0.02; 0.02],0,'LineStyle','none');hold on; p(5).FaceColor = [0.7 0.7 0.7];hold on
    p(1) = plot(x,Amp.Total,'.:','Color',[0 0.4470 0.7410],'markersize',8);hold on
    p(2) =plot(x,Amp.Th_IBW,'k-')
    p(3) =plot(x,Amp.Th_OFW,'r-')
    p(4) = plot(xAnti.IBW_OFW,interp1(x,Amp.Total,xAnti.IBW_OFW),'r.','markersize',22)
    legend(p,'Total','IBW','OFW','$X_{anti}[IBW,OFW]$','$L_{IBW,OFW}$','Location','southeastoutside')
    ylim([0 0.004])
    xlim([x(1) x(end)])
...
ax2 = subplot(2,2,2)
    ...
    p(2) = plot(x,-Kinematic.Free.f1x2.tau+12.4,'r-','LineWidth',2);hold on
    legend(p,'IBW','OFW','Location','West')
    ylim([0 12])
    xlim([x(1) x(end)])
    ylabel('Time (s)')
    set(gca,'fontsize',12)
